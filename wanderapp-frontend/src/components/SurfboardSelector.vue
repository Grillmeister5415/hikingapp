  ...
  <div class="surfboard-selector">
    <div class="input-container" ref="inputContainer">
      ...
      >
        ...
        <!-- Add new surfboard option -->
        <div
          v-if="searchQuery.trim().length > 0 && !isSearching && !searchResults.some(b => b.value.toLowerCase() === searchQuery.trim().toLowerCase())"
          :class="['dropdown-item', 'add-new', { highlighted: highlightedIndex === searchResults.length }]"
          @mousedown.prevent="selectSurfboard(searchQuery.trim())"
...
const props = defineProps({
  ...
  placeholder: {
    type: String,
    default: 'z.B. 6\'2 Shortboard, 9\'0 Longboard'
  }
});
...
const searchSurfboards = async (query) => {
  ...
  try {
    ...
    console.error('[SurfboardSelector] Error searching surfboards:', error);
    searchResults.value = [];
  } finally {
...
const handleKeydown = (event) => {
  ...
  const hasAddNewOption = query.length > 0 &&
                          !isSearching.value &&
                          !searchResults.value.some(b => b.value.toLowerCase() === query.toLowerCase());
